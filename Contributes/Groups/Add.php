...
use packages\ghafiye\{Contributes, group, song, Contribute, contributes\preventRejectException, person};
...
class Add extends Contributes {
	...
	public function buildFrontend(): string {
		...
		$person = new person();
		$person->where("ghafiye_persons.status", person::waitForAccept);
		$person->where("ghafiye_groups_persons.group_id", $this->contribute->groupID);
		if ($person->getOne("ghafiye_persons.*")) {
			$html .= '<div class="row">
				<div class="col-xs-12">
					<div class="alert alert-warning">
						<p>'. translator::trans("ghafiye.contribute.add.group.persons.not.active") . '</p>
					</div>
				</div>
			</div>';
		}
	...
	public function onAccept() {
		...
		$user = $this->contribute->user->id;
		db::join("ghafiye_groups_persons", "ghafiye_groups_persons.person=ghafiye_persons.id", "INNER");
		$person = new person();
		$person->where("ghafiye_persons.status", person::waitForAccept);
		$person->where("ghafiye_groups_persons.group_id", $this->contribute->groupID);
		foreach ($person->get(null, "ghafiye_persons.*") as $person) {
			$contribute = new Contribute();
			$contribute->where("person", $person->id);
			$contribute->where("user", $user);
